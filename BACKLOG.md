...
### April 25, 2026 sprint #4b — Salesforce-style report → dashboard widget flow
- [x] **+ New from report on every dashboard** — the dashboard editor header now has a `+ New from report` primary button alongside `+ Widget`. Opens a modal split view: left = searchable saved-report list, right = widget configurator with live preview. Pick report → pick display (Table / Bar / Horizontal bar / Pie / Metric) → configure columns or group-by/aggregate/value-field → see live preview → "Add to dashboard". Saves a new `custom_widget_definitions` row with `saved_report_id` + `visualization_override` + `config.source='saved_report'` and drops it into the current dashboard. `[BF: 5841f06d follow-up, Lightning-style builder request]`
- [x] **WidgetRenderer report mode** — when `config.source === 'saved_report'`, renderer calls `executeSavedReport` (new `src/lib/reportQuery.js`) and draws the chosen visualisation. Table uses the configured columns; bar / horizontal bar / pie group by the categorical field and aggregate the value field (count / sum / avg / min / max); metric shows a single big number. Shared palette across charts.
- [x] **Shared saved-report executor** — new `src/lib/reportQuery.js` handles `base_entity`, fields list, filters (legacy and grouped), order, limit. Reused by the preview pane, the widget at runtime, and dashboard editor.

### April 25, 2026 sprint #4 — error-surface + restructure pass
- [x] **/coach emails tab error fixed** — Context-to-Include checkboxes were writing to a non-existent `context_include` column; switched to the real `include_*` boolean columns (`include_pain_points`, `include_contacts`, etc.). `updateEmailTemplate` now surfaces any RLS / schema errors instead of silently swallowing them. `[BF: 6c6348ce]`
- [x] **Tasks — reject deal-risk-as-task items** — process-transcript v34: prompt tightened with explicit bans ("validate compelling event", "quantify pain", "confirm budget", etc.), required a specific deliverable noun and named person. Backend post-filter reroutes any pattern-matched item to `ai_memory` as a coaching_observation instead of emitting it as a task. Rep/Prospect tab toggle already in DealDetail tasks tab. `[BF: 59e25522]`
- [x] **Home rename + dashboards-as-tabs** — sidebar now says "Home" (was "Pipeline"), separate "Dashboards" sidebar item removed. `/` already has the tabbed dashboard bar (Pipeline tab + each user dashboard + "+ Dashboard" button). `[BF: 6752de60, 2bf03839, 355923c1]`
- [x] **Coach Admin Rules tab + sliders + 2-col density** — new "Rules" tab between Overview and Prompts hosts the assembled system prompt preview + extraction rules (both moved out of Overview). Research Prompt moved to the Research tab. Scoring weight inputs are now range sliders with live percentage chip. Overview card layout already 2-col. `[BF: 935e4f0a, 51bbf058, 0030c302]`
- [x] **MSP template editor error-surfacing + default attendees** — `updateTemplate` / `updateTemplateStage` / `addTemplateStage` now surface save errors (were silent). New `msp_templates.default_attendees` jsonb column + UI pill picker on each expanded template; new stages auto-inherit the template's default attendee roles. Per-stage color, notes, attendee-roles, and drag-to-reorder were already wired. `[BF: c106b846]`
- [x] **Settings collapsibility** — verified already shipped: every top-level `/settings` section uses `SectionCard` with per-section open/closed state persisted to localStorage. `[BF: deb24ce8]`
