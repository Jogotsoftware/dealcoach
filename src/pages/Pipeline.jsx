...
export default function Pipeline() {
      ...
      <>
        {atRisk.map(d => (
          <div key={d.id} onClick={() => navigate('/deal/' + d.id)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '1px solid ' + T.borderLight, cursor: 'pointer' }}>
            <CompanyLogo logoUrl={d.company_profile?.logo_url} customerLogoUrl={d.customer_logo_url} companyName={d.company_name} size="sm" />
  ...
  // Empty state — only beta users (isDealRoomOnly) see a dedicated welcome card.
  // Full-access users with zero deals fall through to the regular pipeline view
  // (the standard chrome + AddMenu in the header is enough to create their first deal).
  if (isDealRoomOnly && !deals.length && !isManager) return (
...
function DashboardTabView({ dashboard, customWidgetDefs, profile, onManage }) {
  ...
  return (
    <div style={{ padding: '16px 24px' }}>
      ...
      {/* Pipeline-scope chat FAB removed — GlobalChatbot (in Layout) handles
          all chat globally. Two FABs at the same corner stacked the blue
          circle behind the lightbulb. */}
    </div>
  )
}
